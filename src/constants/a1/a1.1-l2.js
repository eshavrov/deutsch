const list = [
  ["Wie geht's?", "Как дела?"],
  ["super", "отлично, супер"],
  ["sehr gut", "очень хорошо"],
  ["gut", "хорошо"],
  ["es geht", "ничего, нормально, пойдёт"],
  ["nicht so gut", "не очень хорошо, так себе"],
  ["die Familie", "семья"],
  ["der Freund", "друг"],
  ["die Freundin", "подруга"],
  ["der Mann", "мужчина, муж"],
  ["Eltern", "родители"],
  ["die Frau", "женщина, жена"],
  ["der Vater", "отец, папа"],
  ["die Mutter", "мама, мать"],
  ["der Sohn", "сын"],
  ["die Tochter", "дочь, дочка"],
  ["Geschwister", "брат и сестра"],
  ["der Bruder", "брат"],
  ["die Schwester", "сестра"],
  ["Großeltern", "бабушка и дедушка, дедушка и бабушка"],
  ["der Großvater", "дедушка"],
  ["der Opa", "дедушка"],
  ["die Großmutter", "бабушка"],
  ["die Oma", "бабушка"],
  ["das Kind", "ребенок"],
  ["das Baby", "младенец"],
  ["das Alter", "возраст"],
  ["der Familienstand", "семейное положение"],
  ["der Geburtsort", "место рождения"],
  ["das Heimatland", "родина"],
  ["der Wohnort", "место жительства"],
  ["leben", "жить"],
  ["Kinder haben", "иметь детей"],
  ["wohnen", "жить, проживать"],
  ["geboren", "родиться"],
  ["geschieden", "разведенный, разведен"],
  ["ledig", "холостой, незамужняя"],
  ["verheiratet", "женат, замужем"],
  ["verwitwet", "вдовец, вдова"],
  ["Wie alt?", "сколько лет"],
  ["wo", "где"],
  ["die Hauptstadt", "столица"],
  ["liegen", "лежать, находиться"],
  ["das Jahr", "год"],
  ["Leute", "люди"],
  ["die Nummer", "номер"],
  ["der Partner", "партнер, компаньон"],
  ["die Partnerin", "партнерша"],
  ["die Party", "вечеринка, компания"],
  ["der Satz", "предложение"],
  ["der Text", "текст"],
  ["die Zahl", "число, цифра"],
  ["ankreuzen", "отметить крестиком, поставить крестик"],
  ["ausfüllen", "заполнять, заполнить"],
  ["haben", "иметь, владеть"],
  ["schreiben", "писать"],
  ["spielen", "играть"],
  ["studieren", "изучать, учиться"],
  ["sich vorstellen", "представляться, представлять себе"],
  ["mein", "мой, моя, моё"],
  ["richtig", "правильно, верно"],
  ["falsch", "неправильно, неверно"],
  ["aber", "но, однако"],
  ["jetzt", "теперь, сейчас"],
  ["lange", "долго, давно"],
  ["oder", "или, либо"],
  ["schon", "уже, ну же"],
  ["sehr", "очень, весьма, крайне"],
  ["viele", "много"],
  ["von", "из, от, с"],
  ["er", "он"],
  ["sie", "она, они"],
  ["wir", "мы"],
  ["ihr", "вы"],
  ["im Moment", "в данный момент, сейчас"],
  ["noch einmal", "еще раз, снова"],
];

export { list };
